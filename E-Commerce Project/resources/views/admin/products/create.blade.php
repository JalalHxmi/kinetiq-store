...
<form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="bg-white rounded shadow p-4 space-y-4">
  ...
  <div class="grid grid-cols-2 gap-4">
    <div>
      ...
      <input type="number" step="0.01" name="price" class="border rounded px-3 py-2 w-full" required>
    </div>
    <div>
      <label class="block text-sm mb-1">Stock</label>
      <input type="number" name="stock" class="border rounded px-3 py-2 w-full" required>
    </div>
  </div>
  <div>
